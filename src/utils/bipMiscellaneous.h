...
namespace bip
{
	namespace misc
	{
        ...
        template< typename InputImageType, typename OutputImageType >
		typename OutputImageType::Pointer allocate_image_vector(typename InputImageType::Pointer img, int length)
		{
		    typename OutputImageType::Pointer alloc_img = OutputImageType::New();
		    typename OutputImageType::RegionType region = img->GetLargestPossibleRegion();
		    alloc_img->SetRegions(region);
		    ...
		    itk::ImageRegionIterator< OutputImageType > iterator(alloc_img, alloc_img->GetLargestPossibleRegion());
            typename InputImageType::PixelType v[length];
        ...
        template< typename InputImageType, typename OutputImageType >
        typename OutputImageType::Pointer rescale_image(
                                     typename InputImageType::Pointer input_img,
        ...
        template< typename InputImageType, typename OutputImageType >
        typename OutputImageType::Pointer cast_image(
                                     typename InputImageType::Pointer input_img
                                     )
        {
        ...
        std::string* get_min_max_intensity( typename InputImageType::Pointer inputImage )
        {
            using MinimumMaximumImageCalculatorType = itk::MinimumMaximumImageCalculator< InputImageType >;
            typename MinimumMaximumImageCalculatorType::Pointer calc = MinimumMaximumImageCalculatorType::New();
            calc->SetImage( inputImage );
            calc->Compute();

            std::string *intensity = new std::string[2];
            intensity[0] = std::to_string((float)calc->GetMinimum());
            intensity[1] = std::to_string((float)calc->GetMaximum());
            return intensity;
        }
