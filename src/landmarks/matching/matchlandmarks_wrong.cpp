                ...
                float                       min_dist)
{
    ...
    if (num_max_matches > 0)
    {
        ...
        for (size_t i = 0; i < landmarks1.size(); ++i) {
            ...
            if(((sqr(static_cast<float>(landmarks1[i].get_location()[0]) - static_cast<float>(landmarks2[j].get_location()[0]))< min_dist))
            && ((sqr(static_cast<float>(landmarks1[i].get_location()[1]) - static_cast<float>(landmarks2[j].get_location()[1]))< min_dist))
            && ((sqr(static_cast<float>(landmarks1[i].get_location()[2]) - static_cast<float>(landmarks2[j].get_location()[2]))< min_dist))){
                continue;
