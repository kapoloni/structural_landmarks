...
void
landmark_detector::
compute(char *pointSetFilename)
{
  // std::cout<< options << "\n";
    if (are_options_set(OPTION_CALCULATE_LANDMARK)){
      compute_landmark_locations();
    }
    else{
      read_landmark_locations(pointSetFilename);
    }

    if (are_options_set(OPTION_CALCULATE_ORIENTATION)){
...
void
landmark_detector::
read_landmark_locations(char *pointSetFilename)
{
    ...
    float coorX, coorY, coorZ, r;

    TImage::IndexType voxelIndex;

    FILE *fp = fopen (pointSetFilename, "r");
    if (fp == NULL)
        throw "Error when opening the file";
    r = fscanf(fp, "%f %f %f\n", &coorX, &coorY, &coorZ);
    while (r != EOF)
    {
        triple<size_t> location(coorX, coorY, coorZ);

        voxelIndex[0] = coorX;      // x position of the voxel
        voxelIndex[1] = coorY;      // y position of the voxel
        voxelIndex[2] = coorZ;      // z position of the voxel
        // Get the index of the current neighbor pixel.

        size_t j = voxelIndex[0] + m_sizes[0] * (voxelIndex[1] + m_sizes[1] * voxelIndex[2]);
        // std::cout << location << "\n";
        triple<float>  features(saliencies_map[j]);
        // triple<float>  features(0);

        temp_landmarks.push_back(landmark(location, features));
        // std::cout << coorX << " " << coorY << " " << coorZ << std::endl;
        r = fscanf(fp, "%f %f %f\n", &coorX, &coorY, &coorZ);
    }
    fclose (fp);
    ...
    for (long i = 0; i <= static_cast<long>(num_landmarks) - 1; ++i){
        m_landmarks.push_back(temp_landmarks[i]);
    }
    std::cout<<"\nNúmero de landmarks lidos: "<<m_landmarks.size()<<std::endl;
...
compute_landmark_orientations()
{
    ...
    const size_t num_bins_azimuth   = 8;
    const size_t num_bins_elevation = (num_dims == 2) ? 1 : 4;
    ...
    for (size_t b = 0, e = 0; e < num_bins[2]; ++e) {
        ...
        for (size_t a = 0; a < num_bins[1]; ++a, ++b) {
            ...
            histogram[b][1] = phi;
            histogram[b][2] = theta;
            // std::cout<< "b "<< b <<" phi "<<phi*180/M_PI << " theta " << theta*180/M_PI << "\n";
        }
    }
        ...
        for (size_t z = zmin; z <= zmax; ++z)
            for (size_t y = ymin; y <= ymax; ++y)
                for (size_t x = xmin; x <= xmax; ++x)
                {
                    ...
                    // histogram[b][0] += magnitude;
                    // histogram[b][0] += 1.0;
                }
                // for (size_t b = 0; b < num_bins_total; ++b)
                //     std::cout<< histogram[b][0] << "\n";
...
compute_landmark_global_descriptors()
{
    ...
    // Get the edge map.
    const size_t total_size = m_sizes[0] * m_sizes[1] * m_sizes[2];
    float *edge_map = read_scalar_map("eigenvalues_0");
    normalize_minmax(edge_map, total_size, 0.0, 1.0);

    // Log-spherical sampling parameters (number of bins).
    const size_t num_bins_radius    = m_nr;
    const size_t num_bins_azimuth   = m_na;
    const size_t num_bins_elevation = (num_dims == 2) ? 1 : (int)m_na/2;
    ...
    for (size_t i = 0; i < m_landmarks.size(); ++i)
    {
        ...
        for (size_t z = zmin; z <= zmax; ++z)
            ...
            normalize_descriptor(histogram);
...
compute_landmark_local_descriptors()
{
    ...
    #ifdef BIP_VERBOSE_MODE
        std::cout << "Computing local descriptors:\n";
    #endif
    ...
    for (size_t i = 0; i < m_landmarks.size(); ++i)
    {
        ...
        const size_t radius = m_local_descr_region_size;
        const size_t zmin   = (z0 < radius) ? 0 : z0 - radius;
        const size_t zmax   = (z0 + radius >= m_sizes[2]) ? m_sizes[2]-1 : z0 + radius;
        const size_t ymin   = (y0 < radius) ? 0 : y0 - radius;
        const size_t ymax   = (y0 + radius >= m_sizes[1]) ? m_sizes[1]-1 : y0 + radius;
        ...
        for (size_t z = zmin; z <= zmax; ++z)
            for (size_t y = ymin; y <= ymax; ++y)
                for (size_t x = xmin; x <= xmax; ++x)
                {
                    ...
                    const float lenght = sqrt(sqr(cart[0]) + sqr(cart[1]) + sqr(cart[2]));

                    // Ignore pixels farther (in rho distance) than the maximum radius.
                    if (lenght >= static_cast<float>(radius))
                        continue;

                    triple<float> sph = normalize(cart);
                    const int b = spider_web_bin(sph, lenght, radius, num_bins);
            ...
            // normalize_vnorm(histogram);
            normalize_descriptor(histogram);
            m_landmarks[i].set_local_descriptor(histogram);
