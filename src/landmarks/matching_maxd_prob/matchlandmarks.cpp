                ...
                float                       min_location_dist)
{
    ...
    if (min_location_dist < 0.0)
        min_location_dist = 0.0;
    ...
    if (num_max_matches > 0)
    {
        ...
        for (size_t k = 0; k < 2; ++k) {
            ...
            for (size_t i = 0; i < (*la).size(); ++i) {
                ...
                for (size_t j = 0; j < (*lb).size(); ++j) {
                    ...
                    //diminuir tempo de matching... testar
                    // if (location_dist > max_dist_area)
                    //     continue;
                    

                    // Get the euclidean distance between the local descriptors.
                    float local_descr_dist = chi_square_dist((*la)[i].get_local_descriptor(),
                    ...
                    // float final_descr_dist = local_descr_dist;
        ...
        for (size_t i = 0; i < landmarks1.size(); ++i) {
            ...
            // if(((abs(static_cast<float>(landmarks1[i].get_location()[0]) - static_cast<float>(landmarks2[j].get_location()[0]))< min_location_dist))
            // || ((abs(static_cast<float>(landmarks1[i].get_location()[1]) - static_cast<float>(landmarks2[j].get_location()[1]))< min_location_dist))
            // || ((abs(static_cast<float>(landmarks1[i].get_location()[2]) - static_cast<float>(landmarks2[j].get_location()[2]))< min_location_dist)))
            //   continue;
